...
public sealed class SpriteBatch : IDisposable {
	...
	private readonly List<SpriteInfo> _spriteQueue = new();
	...
	private sealed class BatchBuffer(VertexBuffer<SpriteVertex> vertexBuffer, IndexBuffer indexBuffer, int spriteCapacity) : IDisposable {
		public VertexBuffer<SpriteVertex> VertexBuffer = vertexBuffer;
		public IndexBuffer IndexBuffer = indexBuffer;
		public readonly int SpriteCapacity = spriteCapacity;
	...
	private readonly struct RetiredPipeline(Pipeline pipeline, int frameIndex) {
	...
	private void ResetBuffersForFrame() {
		...
		foreach (var descriptorSet in descriptorSets.Where(descriptorSet => descriptorSet.Value != VkDescriptorSet.Null)) {
	...
	public SpriteBatch(GraphicsDevice graphicsDevice, ReadOnlySpan<byte> vertexShaderCode, ReadOnlySpan<byte> fragmentShaderCode, int initialSpriteCapacity = 256) {
		...
		for (int i = 0; i < _descriptorSetsInFlight.Length; i++) {
			_descriptorSetsInFlight[i] = new();
		}

		_vertexShaderCode = vertexShaderCode.ToArray();
		_fragmentShaderCode = fragmentShaderCode.ToArray();

		_descriptorSetLayout = new(_device, 0, VkDescriptorType.CombinedImageSampler, VkShaderStageFlags.Fragment);
	...
	public void End() {
		...
		if (_sortMode == SpriteSortMode.Deferred) {
			_spriteQueue.Sort((a, b) => a.LayerDepth.CompareTo(b.LayerDepth));

			foreach (var spriteInfo in _spriteQueue) {
				if (_currentTexture is not null && !ReferenceEquals(_currentTexture, spriteInfo.Texture)) {
					Flush();
					_currentTexture = null;
					_currentBatchDescriptorSet = default;
				}
				EnsureTextureBound(spriteInfo.Texture);
				EnsureSpaceForSprites(1);
				GenerateSpriteVertices(spriteInfo);
				_spriteCount++;
			}
		}
	...
	public void Draw(DrawSettings settings) {
		Vector2 sourceSize =
			GetSourceSize(settings.Texture, settings.SourceRectangle);
		...
		Vector2 finalScale =
			settings.Scale == Vector2.Zero ? Vector2.One : settings.Scale;
	...
	)
	{
		if (!_isActive)
		{
			throw new InvalidOperationException(
				"SpriteBatch.Draw must be called between Begin and End."
			);
		}
		if (texture is null)
		{
		...
		if (_sortMode == SpriteSortMode.Deferred) {
			_spriteQueue.Add(new SpriteInfo {
				Texture = texture,
				Position = position,
				SourceRectangle = sourceRectangle,
				Color = color,
			...
			GenerateSpriteVertices(new SpriteInfo
			{
				Texture = texture,
				Position = position,
				SourceRectangle = sourceRectangle,
	...
	private void GenerateSpriteVertices(SpriteInfo sprite) {
		Vector2 sourceSize = GetSourceSize(sprite.Texture, sprite.SourceRectangle);
		if (sourceSize.X <= 0f ||
			sourceSize.Y <= 0f ||
			sprite.Scale.X == 0f ||
			sprite.Scale.Y == 0f)
		...
		if (sprite.SourceRectangle.HasValue) {
			Rectangle src = sprite.SourceRectangle.Value;
			u0 = src.Left / sprite.Texture.Width;
			v0 = src.Top / sprite.Texture.Height;
			u1 = src.Right / sprite.Texture.Width;
			v1 = src.Bottom / sprite.Texture.Height;
		}
		else {
			u0 = 0f;
			v0 = 1f;
		...
		Vector2 scaledSize = sourceSize * sprite.Scale;
		Vector2 originScaled = sprite.Origin * sprite.Scale;
		...
		topRight += sprite.Position;
		bottomRight += sprite.Position;
		bottomLeft += sprite.Position;
		...
		_vertexScratch[vertexBase + 0] = new SpriteVertex(
			new Vector3(topLeft, sprite.LayerDepth),
			sprite.Color.ToVector4(),
			texCoordTl
		);
		_vertexScratch[vertexBase + 1] = new SpriteVertex(
			new Vector3(topRight, sprite.LayerDepth),
			sprite.Color.ToVector4(),
			texCoordTr
		);
		_vertexScratch[vertexBase + 2] = new SpriteVertex(
			new Vector3(bottomRight, sprite.LayerDepth),
			sprite.Color.ToVector4(),
			texCoordBr
		);
		_vertexScratch[vertexBase + 3] = new SpriteVertex(
			new Vector3(bottomLeft, sprite.LayerDepth),
			sprite.Color.ToVector4(),
			texCoordBl
		);
	}
